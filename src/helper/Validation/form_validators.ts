...
export const length: FormValidator<string, IFormValidatorOptions> = (value, options) => {
...
export const regex:  FormValidator<string, IFormValidatorOptions> = (value, options) => {
...
export const fileType: FormValidator<File, IFormValidatorOptions> = (file, options) => {

    if(!isFile(file)) throw new ValidationError("We need File instance...");

    if(options.fileTypes === undefined) throw new ValidationError("No fileTypes...");

    const fileType = file.type;

    if(!isInArray<string>(fileType, options.fileTypes)) return options.errorMessage;

    return '';
};

export const fileSize: FormValidator<File, IFormValidatorOptions> = (file, options) => {

    if(!isFile(file)) throw new ValidationError("We need File instance...");

    if(options.fileMaxSize === undefined) throw new ValidationError("No fileMaxSize...");
