...
export abstract class PageController<T extends IClasses>
  implements IPageController<T> {
  ...
  protected getStateByChangeIndex = (
    state: IPageState,
    newIndex: number,
    ...
    this.pageClasses.setClassesByActiveIndex(
      newIndex,
  ...
  reducer = (state: IPageState, action: PageAction) => {
    ...
    switch (action.type) {
      case "POPSTATE":
        return this.popstateAC(state, action);
      ...
      case "SHOW_MODAL_TOP":
        return this.showModalFromTopAC(state, action);

      case "SHOW_MODAL_LEFT":
        return this.showModalFromLeftAC(state, action);

      case "HIDE_MODAL":
        return this.hideModalAC(state, action);
    }
  ...
  popstateAC = (state: IPageState, action: PageAction): IPageState => {
    ...
    if (window.history.state && window.history.state.url) {
      const newIndex = this.historyManager.getIndexByUrl(
  ...
  setIndexAC = (state: IPageState, action: PageAction): IPageState => {
    ...
    if (
      action.index >= 0 &&
      action.index <= this.numberOfSections - 1 &&
      action.index !== state.activeSectionIndex
    ) {
      if (this.html === null) throw new Error("No html...");
  ...
  showModalFromTopAC = (state: IPageState, action: PageAction): IPageState => {
    if (
      action.modalType === undefined ||
      action.modalChildrenType === undefined
    )
