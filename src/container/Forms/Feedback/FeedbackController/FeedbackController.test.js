...
describe("FeedbackController", () => {
    ...
    beforeEach(() => {
        ...
        model.inputValidation.mockReturnValue({name: "value", error: "Big error"});
        ...
        controller.setFormState = (callback) => {
            newState = callback({formController: 'ctrl', formElementsState: [
                {name: "value", error: ''},
                {name: "other", error: ''}
            ]});
        };
    ...
    test("onChangeHandler", () => {
        ...
        expect(model.getValidatorsDesc).toHaveBeenNthCalledWith(1, feedbackElements);
        expect(model.inputValidation).toHaveBeenNthCalledWith(1, "target", {hello: "buy"});
