import React, { useMemo } from 'react';
import classes from './Form.module.scss';
import Button from "../UI/Button/Button";

export enum ELEMENT_TYPE{
    INPUT,
    TEXTAREA,
    FILE_INPUT,
    SELECT
}

interface FormProps {
    formError: string;
    formMessage: string;
    onSubmit: (event: {}) => void;
    onClear: (event: {}) => void;
    children: {};

    isSuccess?: boolean;
    onSuccess?: (event: {}) => void;
    isLoading?: boolean;
}

const Form = ({formError, formMessage, onSubmit, onClear, children, isLoading = false, isSuccess = false, onSuccess = undefined} : FormProps) => {

    return (

        <div className={classes.Wrapper}>

            <div className={classes.Form} >

                <form action={"#"} className={classes.FormElements} onSubmit={onSubmit}>

                    { children }

                    { formError && <div className={classes.FormError}>
                        <p>{ formError }</p>
                    </div> }

                    { formMessage && <div className={classes.FormMessage}>
                        <p>{ formMessage }</p>
                    </div> }

                    <div className={classes.Buttons}>

                        { useMemo(() => (
                            <Button
                                label={"Очистить"}
                                type={"OUTLINED"}
                                onClick={onClear}
                                style={{ color: 'rgba(0, 0, 0, 0.3)', borderColor: 'rgba(0, 0, 0, 0.3)'}}
                                disabled={isLoading}
                                isLoading={false}
                            />
                        ), [isLoading])}

                        { useMemo(() => {

                            if(isSuccess) return null;

                            return <Button
                                label={"Отправить"}
                                type={"OUTLINED"}
                                onClick={onSubmit}
                                style={{ color: 'rgba(178, 243, 141, 0.85)', borderColor: 'rgba(178, 243, 141, 0.85)'}}
                                disabled={isLoading}
                                isLoading={isLoading}
                            />
                        }, [isLoading, isSuccess])}

                        { useMemo(() => {

                            if(!isSuccess) return null;

                            return <Button
                                label={"Ок"}
                                type={"OUTLINED"}
                                onClick={onSuccess}
                                style={{ color: 'rgba(178, 243, 141, 0.85)', borderColor: 'rgba(178, 243, 141, 0.85)'}}
                                disabled={false}
                                isLoading={false}
                            />
                        }
                        , [isSuccess])}

                    </div>

                </form>

            </div>


        </div>

    );
};

export default Form;
        